...
#include <linux/of.h> //zte add by T1/T2
#include <linux/of_address.h>//zte add by T1/T2
#include <linux/fs.h> //zte add by T1/T2
...
#ifdef ZTE_HARDWARE_ID_DEF /*zte add for P898A11 T1/T2 begin*/
static char *fw_name_1 = "tfa98xx_T1.cnt";
module_param(fw_name_1, charp, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(fw_name_1, "TFA98xx DSP firmware (container file) name.");

static int board_id = 0;//default value
static bool tmp_sgn = true;//T2 ->true , T1 ->false
...
#endif /*zte add for P898A11 T1/T2 end*/
...
#if 0
//Please export the symbol from q6afe.c
extern int send_tfa_cal_apr(void *buf, int cmd_size, bool bRead) __attribute__((weak));
extern int send_tfa_cal_in_band(void *buf, int cmd_size) __attribute__((weak));
...
#else		
	//result = send_tfa_cal_apr(buffer, (int)size, false); Qch temp-xxx
#endif
...
#else
    (void)recv_data_len;
	//result = send_tfa_cal_apr(buffer, (int)size, true); Qch temp-xxx
#endif
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	//for casa,  pcm id, miid
	nr_controls += 2; /* calibration */
...
#ifdef ZTE_HARDWARE_ID_DEF /*zte add for P898A11 T1/T2 */
	if(tmp_sgn) {
		pr_err("Qch test enter %s tmp_sgn = %d,T2\n",__func__, tmp_sgn);
#endif    
		...
		pr_err("Qch test enter %s tmp_sgn = %d,T1\n",__func__, tmp_sgn);
		return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
		fw_name_1, tfa98xx->dev, GFP_KERNEL,
		tfa98xx, tfa98xx_container_loaded);
	}
#endif
}
...
enum Tfa98xx_Error tfa98xx_adsp_send_calib_values(void)
{
	...
	if (tfa98xx_device_count == bytes[0]) {
		...
		//ret = send_tfa_cal_in_band(&bytes[1], sizeof(bytes) - 1);   QCh temp-xx
		msleep(10);
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	pr_debug("enter tfa98xx_probe QCh \n");
    ...
    pr_info("Container loading requested:Qch %d\n", ret);
	...
	dev_info(codec->dev, "tfa98xx codec registered (%s)",
		tfa98xx->fw.name);
    pr_info("tfa98xx codec registered Qch  (%s)",tfa98xx->fw.name);
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	pr_err("%s Qch reset-gpio:0x%x %d \n",__func__, tfa98xx->reset_gpio, tfa98xx->reset_gpio);
	return 0;
}
	...
	const struct i2c_device_id *id)
{
...
#ifdef ZTE_HARDWARE_ID_DEF /*zte add for P898A11 T1/T2*/ 
		pr_err("%s Qch before reset-gpio:0x%x %d \n",__func__, i2c->addr, tfa98xx->reset_gpio);
		if(!tmp_sgn){
			if(i2c->addr == 0x34) tfa98xx->reset_gpio = 0x12E;
			if(i2c->addr == 0x35) tfa98xx->reset_gpio = 0x186;
		}
		pr_err("%s Qch after reset-gpio:0x%x %d \n",__func__, i2c->addr, tfa98xx->reset_gpio);
#endif
		if (no_start)
			tfa98xx->irq_gpio = -1;
		if (no_reset)
	...
	pr_info("%s Qch i2c-dev-name=%d\n", __func__,i2c->adapter->nr);
	pr_info("%s Qch dai-name=%s\n", __func__,dai->name);
...
static int __init tfa98xx_i2c_init(void)
{
	...
	board_id = request_board_id();
        tmp_sgn= true;   // T2
        if((board_id == 0x108)||(board_id == 0x208)||(board_id == 0x308)) tmp_sgn= false;  // T1
        if(board_id == 0x501)      tmp_sgn= false;  // T0
        if(board_id == 0x601)      tmp_sgn= false;  // T0
	pr_err("QCh board_id = 0x%x,tmp_sgn = %d\n",board_id,tmp_sgn);
#endif
