...
static int zlog_get_already_registered_client(struct zlog_mod_info *mod_info)
{
	...
	for (i = 0; i < MAX_CLIENT_CNT; i++) {
		...
		if ((g_zlog_server.client_list[i].initialized)
				...
				&& (strcmp(g_zlog_server.client_list[i].client_name, mod_info->name) == 0)
				&& (strcmp(g_zlog_server.client_list[i].device_name, mod_info->device_name) == 0)
				&& (strcmp(g_zlog_server.client_list[i].ic_name, mod_info->ic_name) == 0)
				&& (strcmp(g_zlog_server.client_list[i].module_name, mod_info->module_name) == 0)) {
			mutex_unlock(&g_zlog_server.client_list[i].client_lock);
			break;
		} else {
			mutex_unlock(&g_zlog_server.client_list[i].client_lock);
