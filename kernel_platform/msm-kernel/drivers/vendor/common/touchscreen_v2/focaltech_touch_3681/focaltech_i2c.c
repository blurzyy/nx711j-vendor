...
int fts_read(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
    ...
    for (i = 0; i < I2C_RETRY_NUMBER; i++) {
        ...
        if (ret < 0) {
			...
			tpd_cdev->tpd_zlog_error_count->i2c_read_error_count++;
			set_i2c_read_error(tpd_cdev->tpd_zlog_flag);
#endif
...
int fts_write(u8 *writebuf, u32 writelen)
{
    ...
    for (i = 0; i < I2C_RETRY_NUMBER; i++) {
        ...
        if (ret < 0) {
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
			tpd_cdev->tpd_zlog_error_count->i2c_write_error_count++;
			set_i2c_write_error(tpd_cdev->tpd_zlog_flag);
#endif
