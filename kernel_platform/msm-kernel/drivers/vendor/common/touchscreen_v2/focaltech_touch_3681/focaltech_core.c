...
void fts_release_all_finger(void)
{
	...
	for (finger_count = 0; finger_count < max_touches; finger_count++) {
		...
		uf_touch_report(-1, -1, events[finger_count].id);
#endif
...
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC)
			uf_touch_report(events[i].x, events[i].y, events[i].id);
#endif
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(ZTE_ONE_KEY)
...
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC)
			uf_touch_report(-1, -1, events[i].id);
#endif
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(ZTE_ONE_KEY)
...
int fts_ts_resume(struct device *dev)
{
	...
	if (tpd_cdev->tpd_report_wq)
		queue_delayed_work(tpd_cdev->tpd_report_wq, &tpd_cdev->tpd_zlog_check_work, msecs_to_jiffies(2000));
#endif
#ifdef CONFIG_TOUCHSCREEN_UFP_MAC
