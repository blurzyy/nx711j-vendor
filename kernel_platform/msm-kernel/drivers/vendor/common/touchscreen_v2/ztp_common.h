...
#define ZLOG_I2C_R_ERROR			1
#define ZLOG_I2C_W_ERROR			(1 << 1)
#define ZLOG_CRC_ERROR			(1 << 2)
#define ZLOG_FW_UPGRADE_ERROR	(1 << 3)
#define ZLOG_ESD_CHECK_ERROR		(1 << 4)

#define set_i2c_read_error(x)   do { ((x) |= ZLOG_I2C_R_ERROR);	\
	cancel_delayed_work_sync(&tpd_cdev->tpd_zlog_check_work);	\
	queue_delayed_work(tpd_cdev->tpd_report_wq, &tpd_cdev->tpd_zlog_check_work, msecs_to_jiffies(3000));	\
	TPD_ZLOG("set i2c read error flag\n"); } while (0)
#define clean_i2c_read_error(x)  do { ((x) &= (~ZLOG_I2C_R_ERROR)); TPD_ZLOG("clean i2c read error flag\n"); } while (0)
#define is_i2c_read_error(x)  (((x) & ZLOG_I2C_R_ERROR) > 0)

#define set_i2c_write_error(x)   do { ((x) |= ZLOG_I2C_W_ERROR); TPD_ZLOG("set i2c write error flag\n"); \
	cancel_delayed_work_sync(&tpd_cdev->tpd_zlog_check_work);	\
	queue_delayed_work(tpd_cdev->tpd_report_wq, &tpd_cdev->tpd_zlog_check_work, msecs_to_jiffies(3000));	\
	} while (0)
#define clean_i2c_write_error(x)  do { ((x) &= (~ZLOG_I2C_W_ERROR)); TPD_ZLOG("clean i2c write error flag\n"); } while (0)
#define is_i2c_write_error(x)  (((x) & ZLOG_I2C_W_ERROR) > 0)

#define set_crc_error(x)   do { ((x) |= ZLOG_CRC_ERROR); TPD_ZLOG("set crc error flag\n"); } while (0)
#define clean_crc_error(x)  do { ((x) &= (~ZLOG_CRC_ERROR)); TPD_ZLOG("clean crc error flag\n"); } while (0)
#define is_crc_error(x)  (((x) & ZLOG_CRC_ERROR) > 0)

#define set_firmware_upgrade_error(x)   do { ((x) |= ZLOG_FW_UPGRADE_ERROR); TPD_ZLOG("set firmware upgrade error flag\n"); } while (0)
#define clean_firmware_upgrade_error(x)  do { ((x) &= (~ZLOG_FW_UPGRADE_ERROR)); TPD_ZLOG("clean firmware upgrade error flag\n"); } while (0)
#define is_firmware_upgrade_error(x)  (((x) & ZLOG_FW_UPGRADE_ERROR) > 0)

#define set_esd_check_error(x)   do { ((x) |= ZLOG_ESD_CHECK_ERROR); TPD_ZLOG("set esd check error flag\n"); } while (0)
#define clean_esd_check_error(x)  do { ((x) &= (~ZLOG_ESD_CHECK_ERROR)); TPD_ZLOG("clean esd check error flag\n"); } while (0)
#define is_esd_check_error(x)  (((x) & ZLOG_ESD_CHECK_ERROR) > 0)

struct zlog_error_count {
	u32 i2c_read_error_count;
	u32 i2c_write_error_count;
	u32 crc_error_count;
	u32 firmware_upgrade_error_count;
	u32 esd_check_error_count;
};
...
struct ztp_device {
	...
	struct delayed_work tpd_zlog_check_work;
	struct zlog_error_count *tpd_zlog_error_count;
	unsigned int tpd_zlog_flag;
#endif
