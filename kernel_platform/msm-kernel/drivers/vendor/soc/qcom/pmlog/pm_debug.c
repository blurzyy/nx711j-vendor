...
static int tcp_socket_debugfs = 0;
module_param_named(tcp_output_debug,
    tcp_socket_debugfs, int, S_IRUGO | S_IWUSR | S_IWGRP);
...
#define PACKET_PRINT_COUNT_THRESHOLD 6
//zsw ghl added end
...
{
	long curTime;
	if (tcp_socket_debugfs == TCP_IP_LOG_REDUCED) {
...
static unsigned int packet_v4_filter(void *priv, struct sk_buff *skb, const struct nf_hook_state *state, int data_direct)
{
	if (!skb) {
		...
		if ((tcp_socket_debugfs & TCP_IP_LOG_ENABLE) == 0) {
			return NF_ACCEPT;
		}

		if (skb->sk != NULL) {
			uid = skb->sk->sk_uid.val;
		}
		...
		if	(remote_addr == NULL || ipv4_is_loopback(*remote_addr) || (*remote_addr ==  htonl(INADDR_ANY))) {
			return NF_ACCEPT;
		}
		...
		if (iph->protocol == IPPROTO_TCP) {
			...
			pr_info("[IP] %s %s uid = %d, len=%d, (%pI4:%hu %s %pI4:%hu), CWR:%d, ECE:%d, URG:%d, ACK:%d, PSH:%d, RST:%d, SYN:%d, FIN:%d, SEQ:%u, ACK:%u\n",
				...
				tcph->cwr, tcph->ece, tcph->urg, tcph->ack, 
				tcph->psh, tcph->rst, tcph->syn, tcph->fin,
...
static unsigned int packet_v6_filter(void *priv, struct sk_buff *skb, const struct nf_hook_state *state, int data_direct)
{
	if (!skb) {
		...
		if ((tcp_socket_debugfs & TCP_IPV6_LOG_ENABLE) == 0) {
			return NF_ACCEPT;
		}
		...
		if (iph->nexthdr == IPPROTO_TCP) {
			...
			pr_info("[IPV6] %s %s uid = %d, len=%d, (%pI6:%hu %s %pI6:%hu), CWR:%d, ECE:%d, URG:%d, ACK:%d, PSH:%d, RST:%d, SYN:%d, FIN:%d, SEQ:%u, ACK:%u\n",
				TRANS_TYPE_STR[TRANS_TYPE_TCP], DATA_DIRECT_STR[data_direct],
