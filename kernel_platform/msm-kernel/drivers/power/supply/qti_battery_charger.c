...
#define BC_WLS_NU_FW_UPDATE		0X102
#define BC_WLS_NU_VOUT			0x103
#define BC_WLS_NU_VRECT			0x104
#define BC_WLS_NU_USBTYPE		0x105
#define BC_WLS_NU_CEP			0x106
#define BC_WLS_NU_EPP			0x107
#define BC_WLS_NU_SET_VOUT		0x108
#define BC_WLS_NU_I2C_TEST		0x109
#define BC_WLS_NU_IOUT			0x110
#define BC_WLS_NU_VER			0x111
#define BC_WLS_NU_SLEEP			0x112
...
#define MAX_OEM_MSG_SIZE		100

#define UPDATE_SOC_WORK_TIME_MS		10000

#define WLS_ONLINE_DELAY	2
...
enum nu1619_fw_state_type {
	NU1619_FW_STATE_IDLE,
	NU1619_FW_STATE_UPDATE_START,
	NU1619_FW_STATE_UPDATE_DOING,
	NU1619_FW_STATE_UPDATE_SUCCESS,
	NU1619_FW_STATE_UPDATE_FAILED,
	NU1619_FW_STATE_UPDATE_PENDING,
};
struct nu1619_fw_update_status {
	struct pmic_glink_hdr	hdr;
	u32			update_state;
};
struct wls_nu_resp_msg {
	struct pmic_glink_hdr	hdr;
	u32			value;
	u32			ret_code;
};
...
/* add for fast_capacity end */
	...
	u32				wls_nu_sleep;
	u32				wlsonline;
	struct delayed_work	wlsonline_change_work;
	bool				notify_en;
};
...
static int battery_chg_nu1619_fw_update(struct battery_chg_dev *bcdev);
static int battery_chg_wls_nu_props(struct battery_chg_dev *bcdev, u32 opcode, u32 val);
				...
				size_t len)
{
	...
	struct nu1619_fw_update_status *nu1619_fw_update_msg = data;
	struct wls_nu_resp_msg *wls_nu_msg = data;
	struct psy_state *pst;
	bool ack_set = false;
	...
	case BC_WLS_STATUS_GET:
		...
		if (validate_message(resp_msg, len) &&
			...
			if (resp_msg->property_id == WLS_ONLINE) {
				if (bcdev->wlsonline == 1 && pst->prop[WLS_ONLINE] == 0) {
					bcdev->wlsonline = WLS_ONLINE_DELAY;//delaying
					pst->prop[WLS_ONLINE] = 1;
					cancel_delayed_work(&bcdev->wlsonline_change_work);
					schedule_delayed_work(&bcdev->wlsonline_change_work, msecs_to_jiffies(1000 * 2));
					pr_info("BC_WLS_STATUS_GET enter delay state, force to online");
				} else {
					if (bcdev->wlsonline == WLS_ONLINE_DELAY) {
						pst->prop[WLS_ONLINE] = 1;
						pr_info("BC_WLS_STATUS_GET in delay state, force to online");
					} else {
						if(bcdev->wlsonline != pst->prop[WLS_ONLINE]) {
							pr_info("BC_WLS_STATUS_GET wlsonline from=%d,to=%d",bcdev->wlsonline, pst->prop[WLS_ONLINE]);
						}
						bcdev->wlsonline = pst->prop[WLS_ONLINE];
					}
				}
			}
			ack_set = true;
	...
	case BC_DEBUG_MSG:
		...
		break;
	case BC_WLS_NU_FW_UPDATE:
		if (len == sizeof(*nu1619_fw_update_msg)) {
			ack_set = true;
			bcdev->nu1619_fw_update_state = nu1619_fw_update_msg->update_state;
			pr_info("BC_WLS_NU_FW_UPDATE: %d\n", bcdev->nu1619_fw_update_state);
			if (bcdev->nu1619_fw_update_state == NU1619_FW_STATE_UPDATE_PENDING) {
				bcdev->nu1619_fw_update_state = NU1619_FW_STATE_UPDATE_START;
				battery_chg_nu1619_fw_update(bcdev);
				pr_info("BC_WLS_NU_FW_UPDATE: PUSHED firmware Updating %d\n", bcdev->nu1619_fw_update_state);
			}
		} else {
			pr_info("BC_WLS_NU_FW_UPDATE wrong!\n");
		}
		break;
	case BC_WLS_NU_VOUT:
		if (len == sizeof(*wls_nu_msg)) {
			ack_set = true;
			pr_info("BC_WLS_NU_VOUT value=%d", wls_nu_msg->value);
			bcdev->wls_nu_vout = wls_nu_msg->value;
		} else {
			pr_info("BC_WLS_NU_VOUT wrong");
		}
		break;
	case BC_WLS_NU_VRECT:
		if (len == sizeof(*wls_nu_msg)) {
			ack_set = true;
			pr_info("BC_WLS_NU_VRECT value=%d", wls_nu_msg->value);
			bcdev->wls_nu_vrect = wls_nu_msg->value;
		} else {
			pr_info("BC_WLS_NU_VRECT wrong");
		}
		break;
	case BC_WLS_NU_USBTYPE:
		if (len == sizeof(*wls_nu_msg)) {
			ack_set = true;
			pr_info("BC_WLS_NU_USBTYPE value=%d", wls_nu_msg->value);
			bcdev->wls_nu_usbtype = wls_nu_msg->value;
		} else {
			pr_info("BC_WLS_NU_USBTYPE wrong");
		}
		break;
	case BC_WLS_NU_CEP:
		if (len == sizeof(*wls_nu_msg)) {
			ack_set = true;
			pr_info("BC_WLS_NU_CEP value=%d", wls_nu_msg->value);
			bcdev->wls_nu_cep = wls_nu_msg->value;
		} else {
			pr_info("BC_WLS_NU_CEP wrong");
		}
		break;
	case BC_WLS_NU_EPP:
		if (len == sizeof(*wls_nu_msg)) {
			ack_set = true;
			pr_info("BC_WLS_NU_EPP value=%d", wls_nu_msg->value);
			bcdev->wls_nu_epp = wls_nu_msg->value;
		} else {
			pr_info("BC_WLS_NU_EPP wrong");
		}
		break;
	case BC_WLS_NU_SET_VOUT:
		if (len == sizeof(*wls_nu_msg)) {
			ack_set = true;
			pr_info("BC_WLS_NU_EPP ret=%d", wls_nu_msg->ret_code);
		} else {
			pr_info("BC_WLS_NU_EPP wrong");
		}
		break;
	case BC_WLS_NU_I2C_TEST:
		if (len == sizeof(*wls_nu_msg)) {
			ack_set = true;
			pr_info("BC_WLS_NU_I2C_TEST value=%d", wls_nu_msg->value);
			bcdev->wls_nu_i2c_test = wls_nu_msg->value;
		} else {
			pr_info("BC_WLS_NU_I2C_TEST wrong");
		}
		break;
	case BC_WLS_NU_IOUT:
		if (len == sizeof(*wls_nu_msg)) {
			ack_set = true;
			pr_info("BC_WLS_NU_IOUT value=%d", wls_nu_msg->value);
			bcdev->wls_nu_iout = wls_nu_msg->value;
		} else {
			pr_info("BC_WLS_NU_IOUT wrong");
		}
		break;
	case BC_WLS_NU_VER:
		if (len == sizeof(*wls_nu_msg)) {
			ack_set = true;
			pr_info("BC_WLS_NU_VER value=%d", wls_nu_msg->value);
			bcdev->wls_nu_ver = wls_nu_msg->value;
		} else {
			pr_info("BC_WLS_NU_VER wrong");
		}
		break;
	case BC_WLS_NU_SLEEP:
		if (len == sizeof(*wls_nu_msg)) {
			ack_set = true;
			pr_info("BC_WLS_NU_SLEEP value=%d", wls_nu_msg->value);
			bcdev->wls_nu_sleep = wls_nu_msg->value;
		} else {
			pr_info("BC_WLS_NU_SLEEP wrong");
		}
		break;
		...
		if (oem_charger_type != 7) {
			...
			if (bcdev->fast_capacity / 100 != ui_capacity) {
				...
				if (bcdev->fast_capacity / 100 > ui_capacity) {
					...
					if (bcdev->fast_capacity / 100 - ui_capacity > 2)
						schedule_delay = FAST_CAPACITY_INCREASE_DELAY_HIGH * 3;
					else if (bcdev->fast_capacity / 100 - ui_capacity > 1)
						schedule_delay = FAST_CAPACITY_INCREASE_DELAY_HIGH * 2;
					else
				...
				size_t len)
{
	...
	if (pst && pst->psy) {
		...
		pr_info("power_supply_changed,online=%d,type=%d,oemtype=%d,icl=%d,soc=%d,vbat=%d,ibat=%d,temp=%d,wlsonline=%d,WLS_ONLINE=%d,vusb=%d",
...
static int battery_get_fcc_by_thermal_level(struct battery_chg_dev *bcdev, int level)
{
	...
	if (bcdev->num_thermal_levels >= LEVELS_OF_EACH_SCENE * 4) {
		support_wls = true;
	}
	if (support_wls) {
		rc = read_property_id(bcdev, pst, WLS_ONLINE);
		if (rc == 0) {
			wls_online = pst->prop[WLS_ONLINE];
		}
		if (wls_online) {
			baselevel = baselevel + LEVELS_OF_EACH_SCENE * 2;
		}
	}
	...
	pr_info("battery_get_fcc_by_thermal_level levelcount=%d,support_wls=%d,wls_online=%d,screenon=%d,level=%d,calc_level=%d,fcc=%d\n",
		bcdev->num_thermal_levels, support_wls, wls_online, bcdev->screen_is_on, level, calc_level, fcc_ua);
...
int battery_chg_get_wls_nu_cep(char *val, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;

	battery_chg_wls_nu_props(bcdev, BC_WLS_NU_CEP, bcdev->wls_nu_cep);
	pr_info("show wls_nu_cep=%d\n",  bcdev->wls_nu_cep);
	return scnprintf(val, PAGE_SIZE, "%d\n", bcdev->wls_nu_cep);
}

struct zte_misc_ops wls_nu_cep_node = {
	.node_name = "wls_nu_cep",
	.set = NULL,
	.get = battery_chg_get_wls_nu_cep,
	.free = NULL,
	.arg = NULL,
};

int battery_chg_get_wls_nu_iout(char *val, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;

	battery_chg_wls_nu_props(bcdev, BC_WLS_NU_IOUT, bcdev->wls_nu_iout);
	pr_info("show wls_nu_iout=%d\n",  bcdev->wls_nu_iout);
	return scnprintf(val, PAGE_SIZE, "%d\n", bcdev->wls_nu_iout);
}

struct zte_misc_ops wls_nu_iout_node = {
	.node_name = "wls_nu_iout",
	.set = NULL,
	.get = battery_chg_get_wls_nu_iout,
	.free = NULL,
	.arg = NULL,
};

int battery_chg_get_wls_nu_usbtype(char *val, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;

	battery_chg_wls_nu_props(bcdev, BC_WLS_NU_USBTYPE, bcdev->wls_nu_usbtype);
	pr_info("show wls_nu_usbtype=%d\n",  bcdev->wls_nu_usbtype);
	return scnprintf(val, PAGE_SIZE, "%d\n", bcdev->wls_nu_usbtype);
}

struct zte_misc_ops wls_nu_usbtype_node = {
	.node_name = "wls_nu_usbtype",
	.set = NULL,
	.get = battery_chg_get_wls_nu_usbtype,
	.free = NULL,
	.arg = NULL,
};

int battery_chg_get_wls_nu_vout(char *val, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;
	battery_chg_wls_nu_props(bcdev, BC_WLS_NU_VOUT, bcdev->wls_nu_vout);
	pr_info("show wls_nu_vout=%d\n",  bcdev->wls_nu_vout);
	return scnprintf(val, PAGE_SIZE, "%d\n", bcdev->wls_nu_vout);
}

struct zte_misc_ops wls_nu_vout_node = {
	.node_name = "wls_nu_vout",
	.set = NULL,
	.get = battery_chg_get_wls_nu_vout,
	.free = NULL,
	.arg = NULL,
};

int battery_chg_get_wls_nu_vrect(char *val, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;
	battery_chg_wls_nu_props(bcdev, BC_WLS_NU_VRECT, bcdev->wls_nu_vrect);
	pr_info("show wls_nu_vrect=%d\n",  bcdev->wls_nu_vrect);
	return scnprintf(val, PAGE_SIZE, "%d\n", bcdev->wls_nu_vrect);
}

struct zte_misc_ops wls_nu_vrect_node = {
	.node_name = "wls_nu_vrect",
	.set = NULL,
	.get = battery_chg_get_wls_nu_vrect,
	.free = NULL,
	.arg = NULL,
};

int battery_chg_set_wls_boost_en(const char *buf, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;
	int rc;
	bool val;
...
int battery_chg_get_wls_boost_en(char *val, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_WLS];
	int rc;
...
struct zte_misc_ops wls_boost_en_node = {
	.node_name = "wls_boost_en",
	.set = battery_chg_set_wls_boost_en,
	.get = battery_chg_get_wls_boost_en,
	.free = NULL,
	.arg = NULL,
};

int battery_chg_set_wls_nu_fw_update(const char *buf, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;
	int rc = 0;

	if (kstrtou32(buf, 10, &bcdev->nu1619_fw_update_state))
		return -EINVAL;
	if (bcdev->nu1619_fw_update_state == NU1619_FW_STATE_UPDATE_START) {
		rc = battery_chg_nu1619_fw_update(bcdev);
	}

	return 0;
}

int battery_chg_get_wls_nu_fw_update(char *buf, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;
	pr_info("nu1619_fw_update_show nu1619_fw_update_state=%d\n",  bcdev->nu1619_fw_update_state);
	return scnprintf(buf, PAGE_SIZE, "%d\n", bcdev->nu1619_fw_update_state);
}

struct zte_misc_ops wls_nu_fw_update_node = {
	.node_name = "wls_nu_fw_update",
	.set = battery_chg_set_wls_nu_fw_update,
	.get = battery_chg_get_wls_nu_fw_update,
	.free = NULL,
	.arg = NULL,
};

int battery_chg_get_wls_nu_ver(char *buf, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;
	battery_chg_wls_nu_props(bcdev, BC_WLS_NU_VER, bcdev->wls_nu_ver);
	pr_info("show wls_nu_ver=%x\n",  bcdev->wls_nu_ver);
	return scnprintf(buf, PAGE_SIZE, "0x%x\n", bcdev->wls_nu_ver);
}

struct zte_misc_ops wls_nu_ver_node = {
	.node_name = "wls_nu_ver",
	.set = NULL,
	.get = battery_chg_get_wls_nu_ver,
	.free = NULL,
	.arg = NULL,
};

int battery_chg_set_wls_nu_sleep(const char *buf, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;

	if (kstrtou32(buf, 10, &bcdev->wls_nu_sleep))
		return -EINVAL;
	pr_info("store wls_nu_sleep=%d\n",	bcdev->wls_nu_sleep);
	battery_chg_wls_nu_props(bcdev, BC_WLS_NU_SLEEP, bcdev->wls_nu_sleep);

	return 0;
}

int battery_chg_get_wls_nu_sleep(char *buf, const void *arg)
{
	struct battery_chg_dev *bcdev = (struct battery_chg_dev *) arg;
	pr_info("show wls_nu_sleep=%d\n",  bcdev->wls_nu_sleep);
	return scnprintf(buf, PAGE_SIZE, "%d\n", bcdev->wls_nu_sleep);
}

struct zte_misc_ops wls_nu_sleep_node = {
	.node_name = "wls_nu_sleep",
	.set = battery_chg_set_wls_nu_sleep,
	.get = battery_chg_get_wls_nu_sleep,
	.free = NULL,
	.arg = NULL,
};
#endif
...
static int battery_chg_nu1619_fw_update(struct battery_chg_dev *bcdev)
{
	struct nu1619_fw_update_status req_msg = { { 0 } };
	int rc;

	/* Send request to enable notification */
	req_msg.hdr.owner = MSG_OWNER_BC;
	req_msg.hdr.type = MSG_TYPE_NOTIFY;
	req_msg.hdr.opcode = BC_WLS_NU_FW_UPDATE;
	req_msg.update_state = bcdev->nu1619_fw_update_state;

	rc = battery_chg_write(bcdev, &req_msg, sizeof(req_msg));
	if (rc < 0)
		pr_err("battery_chg_nu1619_fw_update Failed rc=%d\n", rc);

	return rc;
}

static ssize_t wls_nu_fw_update_store(struct class *c, struct class_attribute *attr,
				const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static ssize_t wls_nu_fw_update_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static CLASS_ATTR_RW(wls_nu_fw_update);

static int battery_chg_wls_nu_props(struct battery_chg_dev *bcdev, u32 opcode, u32 val)
{
	struct wls_nu_resp_msg req_msg = { { 0 } };
	int rc;

	/* Send request to enable notification */
	req_msg.hdr.owner = MSG_OWNER_BC;
	req_msg.hdr.type = MSG_TYPE_REQ_RESP;
	req_msg.hdr.opcode = opcode;
	req_msg.value = val;
	req_msg.ret_code = 0;
	rc = battery_chg_write(bcdev, &req_msg, sizeof(req_msg));
	if (rc < 0)
		pr_err("battery_chg_wls_nu_get Failed rc=%d\n", rc);

	return rc;
}

static ssize_t wls_nu_vout_store(struct class *c, struct class_attribute *attr,
				const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
	pr_info("store wls_nu_vout=%d\n",  bcdev->wls_nu_vout);
	if (kstrtou32(buf, 10, &bcdev->wls_nu_vout))
		return -EINVAL;
	battery_chg_wls_nu_props(bcdev, BC_WLS_NU_SET_VOUT, bcdev->wls_nu_vout);

	return count;
}

static ssize_t wls_nu_vout_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static CLASS_ATTR_RW(wls_nu_vout);

static ssize_t wls_nu_vrect_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static CLASS_ATTR_RO(wls_nu_vrect);

static ssize_t wls_nu_usbtype_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static CLASS_ATTR_RO(wls_nu_usbtype);

static ssize_t wls_nu_cep_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static CLASS_ATTR_RO(wls_nu_cep);

static ssize_t wls_nu_epp_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
	battery_chg_wls_nu_props(bcdev, BC_WLS_NU_EPP, bcdev->wls_nu_epp);
	pr_info("show wls_nu_epp=%d\n",  bcdev->wls_nu_epp);
	return scnprintf(buf, PAGE_SIZE, "%d\n", bcdev->wls_nu_epp);
}
static CLASS_ATTR_RO(wls_nu_epp);

static ssize_t wls_nu_i2c_test_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
	battery_chg_wls_nu_props(bcdev, BC_WLS_NU_I2C_TEST, bcdev->wls_nu_i2c_test);
	pr_info("show wls_nu_i2c_test=%d\n",  bcdev->wls_nu_i2c_test);
	return scnprintf(buf, PAGE_SIZE, "%d\n", bcdev->wls_nu_i2c_test);
}
static CLASS_ATTR_RO(wls_nu_i2c_test);

static ssize_t wls_nu_iout_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static CLASS_ATTR_RO(wls_nu_iout);

static ssize_t wls_nu_ver_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static CLASS_ATTR_RO(wls_nu_ver);
...
static ssize_t wls_nu_sleep_store(struct class *c, struct class_attribute *attr,
				const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static ssize_t wls_nu_sleep_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static CLASS_ATTR_RW(wls_nu_sleep);
...
static struct attribute *battery_class_attrs[] = {
	...
	&class_attr_wls_nu_fw_update.attr,
	&class_attr_wls_nu_vout.attr,
	&class_attr_wls_nu_vrect.attr,
	&class_attr_wls_nu_usbtype.attr,
	&class_attr_wls_nu_cep.attr,
	&class_attr_wls_nu_epp.attr,
	&class_attr_wls_nu_i2c_test.attr,
	&class_attr_wls_nu_iout.attr,
	&class_attr_wls_nu_ver.attr,
	&class_attr_wls_nu_sleep.attr,
	NULL,
};
...
static void battery_chg_wlsonline_change_work(struct work_struct *work)
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev,
				wlsonline_change_work.work);
	struct psy_state *pst;
	int rc = 0;
	//clean delay state
	bcdev->wlsonline = 0;
	...
	if (bcdev->wlsonline == 0) {
		power_supply_changed(pst->psy);
	}
	pr_info("battery_chg_wlsonline_change_work wlsonline=%d", bcdev->wlsonline);
}
...
/* add for fast_capacity end */
...
#if 0  // error in panel will cause battery wrong,remove
	...
	bcdev->nu1619_fw_update_state = NU1619_FW_STATE_IDLE;
	bcdev->wls_nu_vout = 0;
	bcdev->wls_nu_vrect = 0;
	bcdev->wls_nu_usbtype = 0;
	bcdev->wls_nu_cep = 0;
	bcdev->wls_nu_epp = 0;
	bcdev->wls_nu_i2c_test = 0;
	bcdev->wls_nu_iout = 0;
	bcdev->wls_nu_ver = 0;
	bcdev->wls_nu_sleep = 0;
	bcdev->wlsonline = 0;
	INIT_DELAYED_WORK(&bcdev->wlsonline_change_work, battery_chg_wlsonline_change_work);

#if IS_ENABLED(CONFIG_VENDOR_ZTE_MISC)
	zte_misc_register_callback(&screen_on_node, (void *)bcdev);
	zte_misc_register_callback(&oem_charger_type_node, (void *)bcdev);
	zte_misc_register_callback(&wls_nu_cep_node, (void *)bcdev);
	zte_misc_register_callback(&wls_nu_iout_node, (void *)bcdev);
	zte_misc_register_callback(&wls_nu_usbtype_node, (void *)bcdev);
	zte_misc_register_callback(&wls_nu_vout_node, (void *)bcdev);
	zte_misc_register_callback(&wls_nu_vrect_node, (void *)bcdev);
	zte_misc_register_callback(&wls_boost_en_node, (void *)bcdev);
	zte_misc_register_callback(&wls_nu_fw_update_node, (void *)bcdev);
	zte_misc_register_callback(&wls_nu_ver_node, (void *)bcdev);
	zte_misc_register_callback(&wls_nu_sleep_node, (void *)bcdev);
#endif
